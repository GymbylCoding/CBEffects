...
command and pass a table of parameters. These parameters tell CBE exactly
...
Because there are so many customizable values in a parameter table, CBE
provides something called "presets". These are parameters for a vent that have
the necessary values provided. When you tell CBE what preset to use, those
values become the defaults. Then, you can get a certain effect like flame or
smoke by only providing one parameter.

You're not clamped to that one effect, though: CBE treats any parameters
...
you've provided a preset for CBE to use, just add that value into your
...
To see a list of presets, call CBE.listPresets() and CBE will print all
